...
describe('sculptOptionsOf — the normalisation contract', () => {
  ...
  it('resolves an intent that names neither to the wire default (stamp + soft)', () => {
    expect(sculptOptionsOf(base)).toEqual({ tool: 'stamp', profile: 'soft', spill: 'banded' });
    expect(WIRE_DEFAULT_SCULPT_OPTIONS).toEqual({ tool: 'stamp', profile: 'soft', spill: 'banded' });
  });
