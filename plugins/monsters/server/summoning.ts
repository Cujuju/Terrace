...
import { MAX_LIVING_MONSTERS, SUMMON_ORDER, profileOf, summonRatePerSecond } from './kinds.ts';
 ...
 * never come apart.
 *
 * Returns true if there was something to banish.
 */
export function banish(): boolean {
  if (living === null) return false;
  cooldownSeconds = profileOf(living.kind).respawnCooldownSeconds;
  living = null;
  return true;
}
 ...
 *
 * Kinds are considered in SUMMON_ORDER, and the first one whose lair qualifies
 * AND whose roll fires takes the world's single slot.
 */
function trySummon(world: LairWorld, dt: number): void {
  ...
  for (const kind of SUMMON_ORDER) {
    ...
    if (survey.largestRegionCells < profile.minLairDeepCells) continue;
    if (!rollEvent(summonRatePerSecond(profile), dt)) continue;

    const cell = survey.summonCell;
    ...
    if (cell === null || !isLairCell(world, cell.x, cell.y)) {
      invalidateSurvey();
      return;
    }
...
export function advanceSummoning(world: LairWorld, dt: number): void {
  ...
  if (simSeconds - lastSurveySeconds >= LAIR_SURVEY_INTERVAL_SECONDS) {
    ...
    if (living !== null && survey.occupiedRegionCells < profileOf(living.kind).lairCollapseDeepCells) {
      banish();
    }
  }
 ...
 * it against the world as restored; a monster whose basin was drained by another
 * plugin's migration is then banished on that first tick rather than trusted.
 */
