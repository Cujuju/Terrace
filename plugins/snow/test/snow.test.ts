...
describe("snow siting", () => {
  ...
  it("IGNORES mountains in LOCKED chunks — no side channel on hidden terrain", () => {
    ...
    const world = asSnowWorld(
      worldWithTerrain(
        WORLD_SIZE,
        (x) =>
          x < revealedColumns * CHUNK_SIZE
            ? SEA_LEVEL - BAND_HEIGHT
            : SEA_LEVEL + 8 * BAND_HEIGHT,
  ...
  it("judges an off-map centre by the samples that land in the world, or refuses", () => {
    ...
    expect(
      meanUnlockedHeightUnder(world, -200, WORLD_SIZE + 200, 40),
    ).toBeNull();
  });

  it("sites at exactly the threshold and refuses one unit below it", () => {
    const at = asSnowWorld(
      worldWithTerrain(WORLD_SIZE, () => SNOW_MIN_TERRAIN_HEIGHT),
    );
    const below = asSnowWorld(
      worldWithTerrain(WORLD_SIZE, () => SNOW_MIN_TERRAIN_HEIGHT - 1),
    );
...
describe("the unsited roll (#285)", () => {
  it("hands the roll to the kind called rain, BY NAME, and only after trying", () => {
    ...
    expect(attempts).toBeLessThanOrEqual(
      DISC_SITING_ATTEMPTS * SNOW_ELEVATION_SAMPLES,
    );
