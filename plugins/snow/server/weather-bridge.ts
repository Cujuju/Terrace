// snow → weather, via THE CROSS-PLUGIN DEPENDENCY PATTERN
...
// say to weather is "snow is in the sky", and a hub that resolves later must
// still be told).
//
...
// the register: the world is CALM and nobody is told about the snow. Systems
...
// plugin removed the wind, not the snow.
...
export interface WeatherHubApi {
  ...
  /**
   * Asks the kind called `name` to birth one system now (#285). OPTIONAL on
   * purpose: a weather hub from before 2026-09-02 exports no such thing, and the
   * right degradation is "an unsited snow roll is simply lost" rather than
   * refusing the whole bridge over a missing member.
   *
   * BY NAME AND NOTHING ELSE. This plugin knows the STRING 'rain' and nothing
   * about a rain plugin — no import, no type, no assumption that one exists.
   */
...
/**
 * Hands one lost birth to the kind called `name`. False when no hub is running
 * here, when it is too old to offer the hand-off, or when that kind declined —
 * the roll is lost either way and the caller must not branch on which.
 */
