...
//
// Until then every bolt in this game was a CLIENT decision: each storm rig ran
// its own schedule and flashed on its own random clock, so no two players ever
// saw the same bolt and the server never knew one had happened. That was exactly
// right while lightning was decoration.
//
...
//
// So the strike is the server's, and the client is told. The rig keeps
...
// an instant, so it travels as a one-shot event and is never re-sent.
// ─────────────────────────────────────────────────────────────────────────────
...
/** The slice of the server's WorldApi this file reads. */
export interface StrikeWorld {
  readonly worldSize: number;
  heightAt(x: number, y: number): number;
}

/** A storm, as this file needs to see one. */
 ...
 * ignitions — went up ~5×. A per-storm rate makes "how often does lightning
 * start a fire" a function of the system cap, the world size and the spawner
 * tuning, none of which are lightning decisions. The sky coverage target already
 * normalises weather per WORLD rather than per system; lightning follows the
 * same rule.
 *
 ...
 *
 * 0.06/s, unchanged in value: sized against ONE storm's own life, a system lives
 * DISC_MEAN_LIFETIME_SECONDS (240 s), so a lone storm still throws something
 * like a dozen bolts over its passage — frequent enough that a storm overhead
 * reads as dangerous, rare enough that each one is an event. Far below the
 * client governor's MIN_FLASH_INTERVAL_SECONDS floor of one flash per 3 s, so
 * the photosensitivity limit stays a backstop.
 */
...
// happens to be crossing woodland, which was the difference between fire being a
// mechanic and fire being something a player might never see.
//
// EXPOSED, and the word is doing real work. A storm's bolt is aimed inside its
// own footprint and takes the tallest of a few samples; a dry bolt has no
// footprint, so it is aimed at the most EXPOSED cell in the world — which is not
// the same as the highest. A cell on a plateau is high and dull; a cell that
// stands above what surrounds it is the one lightning finds, and it is the one a
// player reads as "of course that got hit".
// ─────────────────────────────────────────────────────────────────────────────
 ...
 * STRIKE_BUDGET_PER_SECOND (0.06/s) whenever one is in the sky, so dry lightning
 * at 1/240 s adds a few percent to the world's total bolts. It is a punctuation
 * mark, not a second weather system: often enough that a long session sees
 * several, rare enough that one is startling.
 */
 ...
 * without always finding the single highest peak.
 */
 ...
 * terrace step is shaped at, so a cell that stands a band above its surroundings
 * scores as exposed while a gentle rise does not.
 */
 ...
 * highest plateau, prominence alone picks a one-cell pimple in a valley. At 2 a
 * cell standing one band above its neighbours beats a cell two bands higher on
 * flat ground — which is the ordering the phrase "exposed land" means.
 */
 ...
 *
 * Uniform over the DISC, not over the radius: drawing r uniformly would pile two
 * thirds of the samples into the middle third of the storm, so the strike would
 * drift toward the eye of every system. `sqrt` of a uniform is the standard
 * correction and it is one call.
 */
function sampleCell(system: StrikeSource, worldSize: number): { x: number; y: number } | null {
  ...
  // A system may sit partly off the edge of the world (the disc engine's spawn
 ...
 *
 * EVERY LIVING SYSTEM OF THIS PLUGIN THROWS BOLTS, and no filtering by kind is
 * needed any more: this plugin IS the kind that has lightning, which is exactly
 * why it is a plugin of its own rather than a flag on rain. The storms share
 * STRIKE_BUDGET_PER_SECOND by intensity (see its doc comment), so one that is
 * still gathering or already dissipating gets proportionally less of it, one at
 * zero gets none, and stacking storms never multiplies the world's bolts.
 */
export function rollStrikes(
  ...
  // The budget's denominator: total storm intensity in the sky this step,
  // floored at 1 so a lone storm below full strength is not handed the whole
  // budget.
