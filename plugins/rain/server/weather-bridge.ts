// rain → weather, via THE CROSS-PLUGIN DEPENDENCY PATTERN
...
// say to weather is "rain is in the sky", and a hub that resolves later must
// still be told).
//
...
// the register: the world is CALM and nobody is told about the rain. Systems
// still gather, sit and fade in place and are still broadcast to clients, so it
// still rains — it just does not drift, and fire does not learn that the ground
// is wet. That is the right failure mode: a self-hoster who removed the weather
// plugin removed the wind, not the rain.
...
export const WEATHER_UNAVAILABLE_WARNING =
  '[rain] weather plugin not available — rain will gather where it forms and never drift';
