// Scratch check (uncommitted): bake the shark from its .glb and report what the
...
// the procedural shark (client/species/.old-shark.ts, a `git show HEAD:` copy)
...
import { SHARK_ASSET, SHARK_ENVELOPE, buildShark } from './client/species/shark.ts';
...
const url = new URL('./client/assets/shark.glb', import.meta.url);
const buffer = await readFile(url);
const bytes = buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength);
const asset = await parseRigAsset(bytes, 'shark.glb');
installSpeciesAsset(SHARK_ASSET, asset);
console.log('installSpeciesAsset: accepted shark.glb');

const authored = buildShark(null as never);
const blueprint = bakeRig(authored.root);
report('GLB shark', blueprint);
...
console.log('  declared SHARK_ENVELOPE:', JSON.stringify(SHARK_ENVELOPE));
...
if (process.argv.includes('--old')) {
  const { buildShark: buildOldShark } = await import('./client/species/.old-shark.ts');
  ...
  const old = buildOldShark(pool);
  const oldBlueprint = bakeRig(old.root);
  report('procedural shark (HEAD)', oldBlueprint);
