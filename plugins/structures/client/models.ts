    ...
    apply(placements: readonly StructurePlacement[]): void {
      ...
      for (const placement of placements) {
        ...
        for (let partIndex = 0; partIndex < parts.length; partIndex++) {
          ...
          let count = counts[placement.tier][partIndex];
          // Capacity (STRUCTURES_CAP × localMatrices.length, see the
          // allocation above) covers every placement the caller can hand in:
          // the server-side registry itself never exceeds STRUCTURES_CAP
          // structures, so `count` cannot outrun `mesh.instanceMatrix`.
          ...
          counts[placement.tier][partIndex] = count;
        }
      }

      for (let tier = 0; tier < meshesByTier.length; tier++) {
        const meshes = meshesByTier[tier];
        for (let partIndex = 0; partIndex < meshes.length; partIndex++) {
          const mesh = meshes[partIndex];
          mesh.count = counts[tier][partIndex];
          ...
          // change, and frustum culling against a stale sphere makes a
          // building vanish when the camera moves.
